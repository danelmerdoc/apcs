...
public class StringLooper {
    ...
    public String toVariablesFormat() {
        ...
        while (curPos < source.length()) {
            accumulator += source.substring( curPos, curPos + 1).equals("_") ?
                    source.substring(++curPos, ++curPos).toUpperCase() : source.substring(curPos, ++curPos).toLowerCase();
        }
    ...
    public String toConstantsFormat() {
        ...
        int curPos = 0;
        while (curPos < source.length()) {
            accumulator += source.substring( curPos, curPos + 1).toUpperCase().equals(source.substring(curPos,curPos + 1)) ?
            "_" + source.substring(curPos, ++curPos) : source.substring(curPos, ++curPos).toUpperCase();
        }
        return accumulator;
    }
