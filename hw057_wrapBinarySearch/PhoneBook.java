...
public class PhoneBook{
    ...
    public/* instance */ String getPhone_whileStyle( String name) {
        ...
        int lowInclusive, highExclusive, midOffset;
        
        // stub values - no longer stubs
        highExclusive = entries.length;
        lowInclusive = 0;
        midOffset = (highExclusive - lowInclusive)/2;
        ...
        // loop through a binary search
         ...
         while (lowInclusive < highExclusive) {
          midOffset = (lowInclusive + highExclusive) / 2;
          int comparison = entries[midOffset].getName().compareTo(name);
          if (comparison == 0) {
              return entries[midOffset].getPhone();
          } else if (comparison > 0) {
              highExclusive = midOffset;
          } else {
              lowInclusive = midOffset + 1;
          }
      }
